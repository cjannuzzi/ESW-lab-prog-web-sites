...
function showAllTodos() {
  ...
  if (todos.length === 0) {
    todos_list_body.innerHTML = `<tr><td colspan="5" class="text-center">Nenhuma tarefa encontrada</td></tr>`;
    return;
  }
  ...
  todos.forEach((todo) => {
    todos_list_body.innerHTML += `
            <tr class="todo-item" data-id="${todo.id}">
                <td>${todo.task}</td>
                <td>${todo.dueDate || "Sem data de vencimento"}</td>
                <td>${todo.status}</td>
                <td>
...
function clearAllTodos() {
  if (todos.length > 0) {
    ...
    showAlertMessage("Não há tarefas para limpar", "error");
  }
}
